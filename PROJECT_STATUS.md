...
- Flutter Drift 已升级至本地 schema v6：单图/三图媒体物化、流式 SHA-256、EXIF 方向/尺寸、方向唯一、ROI 边界持久化、完整采集组入队，以及认证上下文、组织主数据游标缓存均具备自动化证据；上传队列现已实现 create/blob/manifest/commit 的持久状态机、租约、失败分流和 Commit 后才标记 synced，待 Compose/真机联调。
...
| Flutter Android | `flutter analyze` 通过；20 项测试通过；`flutter build apk --debug` 成功，debug APK 已生成 |
...
| Flutter 离线采集与上传恢复 | analyze 通过；20 项测试通过；debug APK 构建成功；10MB 流式物化、方向唯一、ROI、本地入队和三视图剩余 Blob 续传均覆盖；realme GT 7 Pro 已验证单图/三图拍摄和入队、不显示数量、三图 left 后强杀恢复到 center，以及登录、同步、单图上传、断网保留、强杀恢复和恢复网络后提交闭环 |
...
- P0 上传包 create/blob/manifest/commit 的 Controller/Application/Infrastructure、事务 Outbox 记录、MinIO 暂存提升和 MySQL 集成测试，以及环境变量一次性 Bootstrap 管理员、login/refresh/logout/me、JWT、刷新令牌轮换和组织隔离的主数据全量/增量同步已实现；Flutter 已接入安全令牌存储、登录/刷新/登出、7 天离线会话恢复、真实主数据缓存同步，以及持久化 Outbox 上传状态机。真实 Outbox 派发、完整 RBAC，以及 Flutter 上传的 Compose/真机 E2E 尚未实现。
