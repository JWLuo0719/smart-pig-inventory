...
class UploadApi {
  UploadApi({required String baseUrl, required String accessToken})
      : _dio = Dio(BaseOptions(baseUrl: baseUrl, headers: <String, String>{'Authorization': 'Bearer $accessToken'}));

  final Dio _dio;

  Future<void> upload(CapturePackageDraft draft) async {
    final Map<String, Object?> manifest = await draft.toManifest();
    final List<Map<String, Object?>> media = List<Map<String, Object?>>.from(manifest['assets']! as List<Object?>);
    final Options idempotent = Options(headers: <String, String>{'X-Idempotency-Key': draft.idempotencyKey});
    ...
    final Set<String> uploaded = <String>{
      for (final Object? item in package.data['existingAssets'] as List<Object?>) item as String,
    };
    ...
    await _dio.put('/api/v1/upload-packages/$serverPackageId/manifest', data: manifest, options: idempotent);
    await _dio.post('/api/v1/upload-packages/$serverPackageId/commit', options: idempotent);
  }
}
