  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      ...
      child: Container(
        ...
        child: Row(
          children: <Widget>[
            Icon(offline ? Icons.cloud_off_outlined : Icons.cloud_done_outlined, color: color, size: 18),
            const SizedBox(width: 8),
            Expanded(
              child: Text(
                offline ? '离线模式 · $pending 个采集包安全保存在本机' : '网络正常 · 上传队列已同步',
                style: TextStyle(color: offline ? const Color(0xFF805800) : AppColors.herdTeal, fontWeight: FontWeight.w700, fontSize: 12),
              ),
            ),
          ],
        ),
